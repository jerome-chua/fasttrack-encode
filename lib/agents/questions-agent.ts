...
const getInsightsFromAgent = new FunctionTool({
  name: "get_insights_from_agent",
...
const getDailySummaryFromAgent = new FunctionTool({
  name: "get_daily_summary_from_agent",
...
const getUserContext = new FunctionTool({
...
export const questionsAgent = new LlmAgent({
  name: QUESTIONS_AGENT_NAME,
  model: "gemini-2.5-flash",
  description: "Routes questions to specialized agents or answers general nutrition/fasting questions directly.",
  instruction: QUESTIONS_AGENT_INSTRUCTION,
  tools: [getInsightsFromAgent, getDailySummaryFromAgent, getUserContext],
});
